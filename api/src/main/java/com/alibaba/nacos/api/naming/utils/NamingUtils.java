...
public class NamingUtils {
    ...
    public static String getCompletedPattern(final String namespaceId, final String groupedPattern) {
    ...
    public static String getNamespaceId(String completedPattern) {
    ...
    public static String getPattern(String completedPattern) {
        if (StringUtils.isBlank(completedPattern)) {
            return StringUtils.EMPTY;
        }
        if (!completedPattern.contains(Constants.NAMESPACE_ID_SPLITER)) {
     ...
     * @param completedPattern a set of all fuzzy subscribe pattern(with namespace id)
    ...
    public static Set<String> filterPatternWithNamespace(String namespaceId, Set<String> completedPattern) {
        ...
        for (String each : completedPattern) {
            String eachId = getNamespaceId(each);
            if (namespaceId.equals(eachId)) {
                patterns.add(getPattern(each));
            }
        }
        return patterns;
    }
    ...
    public static String getGroupedPattern(final String matchPattern, final String matchType) {
        if (StringUtils.isBlank(matchPattern) && !matchType.equals(Constants.FuzzySubscribe.MATCH_ALL)) {
        ...
        } else if (matchType.equals(Constants.FuzzySubscribe.MATCH_ALL)) {
            return Constants.FuzzySubscribe.MATCH_ALL;
        }
     ...
     */
    public static String getMatchType(String groupedPattern) {
        if (StringUtils.isBlank(groupedPattern)) {
            return StringUtils.EMPTY;
        }
        if (!groupedPattern.contains(Constants.MATCH_PATTERN_SPLITER)) {
    ...
    /**
     * Given a service, and a list of fuzzy match patterns, return the patterns that the service can match.
     *
     * @param serviceName service Name
     * @param groupName group Name
     * @param fuzzySubscribePattern a list of completed fuzzy subscribe patterns
     * @return the patterns list that the service can match.
     */
    public static Set<String> getServiceMatchedPatterns(String serviceName, String groupName, Collection<String> fuzzySubscribePattern) {
        if (fuzzySubscribePattern == null || fuzzySubscribePattern.isEmpty()) {
            return new HashSet<>(1);
        }
        Set<String> matchedPatternList = new HashSet<>();
        for (String eachPattern : fuzzySubscribePattern) {
    ...
    /**
     * Given a list of service's name, and a fuzzy match pattern, return the services that can match the pattern().
     *
    ...
    public static boolean isMatchPattern(String serviceName, String groupName, String serviceNamePattern, String groupNamePattern) {
        ...
        String serviceMatchType = getMatchType(serviceNamePattern);
        // Only support prefix match or all match right now
        // Only support fixed group name right now
        if (serviceMatchType.equals(Constants.FuzzySubscribe.MATCH_ALL)) {
            return groupName.equals(groupNamePattern);
        } else if (serviceMatchType.equals(Constants.FuzzySubscribe.MATCH_PREFIX)) {
